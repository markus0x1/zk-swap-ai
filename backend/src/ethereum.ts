...
export const getDexState = async (
  exchange: 'A' | 'B',
): Promise<{
  decimals0: string;
  decimals1: string;
...
export const getDy = async (
  ...
  return exchange === 'A'
    ? dexAContract.get_dy(dx, inToken)
    : dexBContract.get_dy(dx, inToken);
};

export const verifyProof = (proof: Proof, publicSignals: string[]) => {
  return verifierContract.verifyProof(
    proof._pA,
    proof._pB,
    proof._pC,
    publicSignals,
